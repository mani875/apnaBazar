...
export class ProductsComponent implements OnInit {
  ...
  constructor(public fb: FormBuilder,public filter: FormBuilder,private readonly route: ActivatedRoute,private modalService: NgbModal, private readonly router: Router) {
  ...
  this.searchForm=this.filter.group({
    ...
    this.searchProducts.forEach(x=>{
      ...
      this.category.push(this.product.filter(x => x.category == this.category));
    })
  }
    ...
    search(){
      if(this.searchForm.controls.priceRange.value && this.searchForm.controls.categories.value){
      this.searchProducts= this.product.filter(item=>item.category===this.searchForm.controls.categories.value).
      filter(item=>{
